...
logging.getLogger("example_service").setLevel(logging.INFO)
logger = logging.getLogger("example_service")

class ExampleException:
    """Base exception classes for example operations"""
    class UploadError(AppException):
        def __init__(self, message: str = "Error processing example upload"):
...
class ExampleService:
    ...
    async def create_example(self, example: ExampleCreate, current_user: User) -> APIResponse:
        """Create example with profile and vector store entry"""
    ...
    async def get_example(self, example_id: str, current_user: Union[User, str], include: Optional[List[str]] = None) -> APIResponse:
    ...
    async def delete_example(self, example_id: str, current_user: User) -> APIResponse:
        """Delete example and related data"""
        async with self.db.session() as session:
            async with session.begin():
                try:
                    pass
